    ...
    <table class="w-full border-collapse">
        ...
        <tbody>
            ...
            <tr class="border-b hover:bg-gray-100">
                {{-- ID formateado con ceros a la izquierda --}}
                <td class="px-4 py-2">
                    {{ str_pad($empleado->id, 8, '0', STR_PAD_LEFT) }}
                </td>
                ...
                <td class="px-4 py-2">
                    {{ $empleado->telefono ?? $empleado->telefono_movil }}
                </td>

                {{-- Hora de entrada formateada (HH:mm) --}}
                <td class="border px-4 py-2">
                    {{ $empleado->hora_entrada_contrato
                        ? \Carbon\Carbon::createFromFormat('H:i:s', $empleado->hora_entrada_contrato)->format('H:i')
                        : '-' }}
                </td>
            </tr>
            @endforeach
