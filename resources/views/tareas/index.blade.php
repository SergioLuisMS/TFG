...
<div class="space-y-6" x-data="cronometroManager()" x-init="inicializar()">
    ...
    {{-- Lista de órdenes y sus tareas asociadas --}}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @foreach($ordenes as $orden)
        <div class="bg-white rounded-lg p-4 border border-gray-300 hover:shadow-xl hover:scale-[1.01] transition">
            ...
            {{-- Tarea en curso con cronómetro en vivo --}}
            @if ($mostrarCronometroEnVivo)
            <div class="border-2 {{ $borderColor }} rounded-lg p-3 mb-2 bg-gray-50 hover:shadow-md transition"
                 data-id="{{ $tarea->id }}"
            ...
            @else
            {{-- Tarea con tiempo real registrado --}}
            <div class="border-2 {{ $borderColor }} rounded-lg p-3 mb-2 bg-gray-50 hover:shadow-md transition">
                <p class="text-sm font-mono text-gray-700 mt-2">
                    Tiempo actual:
                    @php
            ...
            @endif
                ...
                <div class="flex justify-between items-center mt-2">
                    ...
                    {{-- Botones para iniciar/finalizar tarea --}}
                    <div class="text-sm">
                        @if($tarea->estado === 'Asignada')
                            <form method="POST" action="{{ route('tareas.marcarEnCurso', $tarea) }}">
                                @csrf
                                <button type="submit" class="bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700">Iniciar</button>
                            </form>
                ...
                <div class="mt-3 text-sm text-gray-800 border-t pt-2">
                    ...
                    <ul class="space-y-2">
                        ...
                        <li class="border rounded bg-gray-50 p-2">
                            ...
                            <div class="text-gray-700 mt-1">{{ $comentario->contenido }}</div>

                            {{-- Valoración del comentario por parte del administrador --}}
                ...
                {{-- Tiempo final con opción de edición --}}
                ...
                <div class="relative group text-sm text-red-700 font-mono mt-2">
                    ...
                    <form action="{{ route('tareas.actualizarTiempo', $tarea->id) }}" method="POST"
                          class="hidden mt-1" id="form-tiempo-{{ $tarea->id }}">
                        @csrf
...
{{-- Script de control de cronómetros en vivo y edición de tiempo --}}
<script>
    ...
    function cronometroManager() {
        return {
            ...
            cronometros: {},
            inicializar() {
                // Inicializa todos los cronómetros activos en las tareas en curso
